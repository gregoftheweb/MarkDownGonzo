        ...
        <section className="document-area">
          ...
          {toolbarOpen && <div className="toolbar" role="toolbar" aria-label="Formatting">
            <label className="select-control font-select"><span className="sr-only">Document font</span>
              <select value={editorFont} onChange={() => undefined}>{config.fonts.families.map((font) => <option value={font} key={font}>{font === "sans-serif" ? "Omarchy Sans" : font}</option>)}</select><ChevronDown size={14} />
            </label>
